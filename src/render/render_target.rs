...
void main() {
  ...
  // TODO: Cache this
  fn get_rectangle_shape(&mut self, _component: &Component, size: &Rect) -> Result<(IndexBuffer<u8>, VertexBuffer<Vertex2D>)> {
    //self.left += 1f32;
    let (left, top, right, bottom) = size.calculate_screen_positions();

    Ok((
      // TODO: This indexbuffer is probably always the same, it might be faster to cache it / put it in the shader
  ...
  fn draw_component(&mut self, target: &mut Render, frame: &mut Frame, component: &Component, parent_size: &Rect) -> Result<()>{
    ...
    for child in &component.children {
      let child = try_get!(target.get_component_by_id(*child), could_not_find_component(*child));
  ...
  pub fn render(&mut self, target: &mut Render) -> Result<()> {
    ...
    let (screen_width, screen_height) = try_get!(try_get!(self.display.get_window(), no_window()).get_inner_size_pixels(), no_inner_pixel_size());
    let (screen_width, screen_height) = (screen_width as SizeType, screen_height as SizeType);
    let screen_size = Rect { x: 0, y: 0, width: screen_width, height: screen_height, screen_width: screen_width, screen_height: screen_height };
