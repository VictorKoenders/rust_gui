...
impl Rect {
  ...
  pub fn left(&self) -> SizeType { self.x }
  pub fn right(&self) -> SizeType { self.x + self.width }
  pub fn top(&self) -> SizeType { self.y }
  pub fn bottom(&self) -> SizeType { self.y + self.height }
  ...
  pub fn calculate_from_parent(parent: &Rect, component: &Component) -> Rect {
    let left = {
      if component.left.is_known() {
        ...
        let right = component.right.get_pixel_position_reversed(parent.left(), parent.right());
    ...
    let top = {
      if component.top.is_known() {
        ...
        let bottom = component.bottom.get_pixel_position_reversed(parent.top(), parent.bottom());
